...
import { rejectClaimSchema } from "@/server/validation/claim.schemas";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const body = rejectClaimSchema.parse(await request.json());
    return NextResponse.json(await getApprovalService().rejectClaim(claimId, body, user));
  } catch (error) {
