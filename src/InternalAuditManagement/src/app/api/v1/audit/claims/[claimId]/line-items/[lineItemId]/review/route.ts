...
import { auditLineReviewSchema } from "@/server/validation/claim.schemas";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const body = auditLineReviewSchema.parse(await request.json());
    return NextResponse.json(await getAuditService().reviewLineItem(claimId, lineItemId, body, user));
  } catch (error) {
